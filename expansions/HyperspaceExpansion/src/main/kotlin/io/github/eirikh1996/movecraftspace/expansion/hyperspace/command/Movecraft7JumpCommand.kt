...
object Movecraft7JumpCommand : CommandExecutor {
    override fun onCommand(sender: CommandSender, cmd: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (navComputersOnCraft.isEmpty()) {
            sender.sendMessage(COMMAND_PREFIX + ERROR + "There are no navigation computers on this craft")
            return true
        }
        val typeName = craft.type.getStringProperty(CraftType.NAME)
        if (!ExpansionSettings.allowedCraftTypesForJumpCommand.contains(typeName)) {
            sender.sendMessage(COMMAND_PREFIX + ERROR + "Craft type " + typeName + " is not allowed for hyperspace travel using jump command")
            return true
        }
        ...
        val range = HyperdriveManager.getMaxRange(craft.hitBox.asSet(), craft.world)
        ...
        val origin = midpoint.toBukkit(craft.world)
        (HyperspaceManager.processor as HyperspaceProcessor).scheduleHyperspaceTravel(craft, origin, origin.clone().add(distanceVec))
        return true
    }
}