...
class HyperspaceExpansion : Expansion(), SelectionSupported {
    ...
    override fun enable() {
        ...
        saveResource("beaconstructure.yml", false, "beaconstructure.yml")
        if (!File(dataFolder, "hyperdrives").exists()) {
            for (i in 1..2) {
                saveResource("hyperdrives/ExampleDrive" + i + ".yml", false, "hyperdrives/ExampleDrive" + i + ".yml")
            }

        }
        if (!File(dataFolder, "gravitywells").exists()) {
            for (i in 1..2) {
                saveResource("gravitywells/ExampleWell" + i + ".yml", false, "gravitywells/ExampleWell" + i + ".yml")
            }
